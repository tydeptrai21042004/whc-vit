...
class Trainer():
    ...
    def train_classifier(self, train_loader, val_loader, test_loader):
        ...
        for epoch in range(total_epoch):
            ...
            self.eval_classifier(val_loader, "val", False)
        ...
        # Restore the best-validation trainable parameters, then evaluate test once.
        ...
        if test_loader is not None:
            self.model.eval()
            self.evaluator.update_iteration(best_epoch - 1 if best_epoch > 0 else 0)
            self.eval_classifier(test_loader, "test", True)
            test_name = "test_" + test_loader.dataset.name
            try:
                test_metric = float(self.evaluator.results[f"epoch_{best_epoch - 1}"]["classification"][test_name]["top1"])
